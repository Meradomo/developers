...
1. They get an email: **"Kamran shared Music with you"**, naming your app and its own
...
| Call | Does |
|---|---|
| `GET /people` | Everyone with access, and the apps you may hand over |
| `POST /people/invite` | Invite by email, granting your app in the same step |
| `POST /people/grant` | Give or withdraw one app for one person |
| `POST /people/revoke` | Remove somebody from the computer entirely |

Full request and response shapes are in the **API reference** tab, under Endpoints.
...
```json
{
  "name": "kamran",
  "members": [
    { "email": "kamran@example.com", "accountId": "8dc…", "role": "owner",
      "status": "active", "apps": [] },
...
{ "email": "sam@example.com", "status": "pending",
  "url": "https://music.kamran.meradomo.com" }
```
