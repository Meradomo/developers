...
A Meradomo address looks like `example.meradomo.com`. A custom domain lets the same computer
answer at a hostname the user owns — `files.example.com` — with the same apps, the same identity
...
| Type | Name | Points to |
| --- | --- | --- |
| CNAME | `files.example.com` | `example.meradomo.com` |
| CNAME | `_acme-challenge.files.example.com` | `_acme-challenge.example.meradomo.com` |
...
Your app is reached at `<your-app>.files.example.com` exactly as it was at
`<your-app>.example.meradomo.com`. The same verified `X-Meradomo-*` headers arrive, injected by
